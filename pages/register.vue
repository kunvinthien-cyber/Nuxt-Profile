  ...
  <NuxtLayout name="auth">
    <div class="animate-fade-up">
      <div class="mb-8">
        ...
        <p class="text-surface-400 mt-1.5 text-sm">Get started for free — no credit card required</p>
      </div>
      ...
      <Transition name="fade">
        <div v-if="success" class="mb-5 flex items-start gap-3 p-4 rounded-xl bg-green-500/10 border border-green-500/20">
          ...
          <div>
            <p class="text-sm font-medium text-green-400">Account created!</p>
            <p class="text-xs text-green-500/80 mt-0.5">Check your email to confirm your account.</p>
          </div>
        </div>
      </Transition>
      ...
      <form v-if="!success" class="space-y-4" @submit.prevent="handleRegister">
        ...
        <div>
          ...
          <!-- Password strength -->
          <div class="flex gap-1 mt-2" v-if="form.password">
            <div
              v-for="i in 4" :key="i"
              class="flex-1 h-1 rounded-full transition-all duration-300"
              :class="i <= passwordStrength ? strengthColors[passwordStrength - 1] : 'bg-surface-800'"
            />
          </div>
          <p v-if="form.password" class="text-xs mt-1" :class="strengthTextColors[passwordStrength - 1]">
        ...
        <div>
          ...
          <input
            ...
            class="input-field"
            :class="{ 'border-red-500/50 focus:border-red-500/50 focus:ring-red-500/20': form.confirmPassword && form.password !== form.confirmPassword }"
            required
            autocomplete="new-password"
          />
        ...
        <div class="flex items-start gap-2.5 py-1">
          <input
            ...
            v-model="form.terms"
            type="checkbox"
            class="w-4 h-4 rounded border-surface-600 bg-surface-800 text-brand-500 focus:ring-brand-500/30 cursor-pointer mt-0.5"
            required
          />
          <label for="terms" class="text-sm text-surface-400 cursor-pointer leading-relaxed">
            I agree to the
        ...
        <button type="submit" class="btn-primary w-full" :disabled="loading || (form.password !== form.confirmPassword && !!form.confirmPassword)">
      ...
      <!-- Divider -->
      <div v-if="!success" class="flex items-center gap-4 my-6">
        <div class="flex-1 divider" />
        <span class="text-xs text-surface-500 font-medium">or sign up with</span>
        <div class="flex-1 divider" />
      </div>

      <!-- OAuth -->
      <div v-if="!success" class="grid grid-cols-2 gap-3">
...
const { signInWithOAuth, getOAuthErrorMessage } = useAuth()
